...
+
Sie können die Ressourcengruppe durchsuchen, indem Sie den Namen der Ressourcengruppe in das Suchfeld eingeben, oder indem Sie auf klicken image:../media/filter_icon.gif["Filtersymbol"] und das Tag auswählen. Sie können dann auf image:../media/filter_icon.gif["Filtersymbol"][] klicken, um den Filterbereich zu schließen.
